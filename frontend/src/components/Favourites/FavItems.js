...
export default function Favitems(props) {
  ...
  const [cartItems, setCartItems] = useState(fav.getfav());
  ...
  const handleChange = (index) => (event) => {
    ...
    fav.updatefav(index, event.target.value);
  };

  const getTotal = () => {
    return favItems.reduce((a, b) => {
      return a + b.quantity * b.product.price;
    }, 0);
  };

  const removeItem = (index) => (event) => {
    let updatedCartItems = fav.removeItem(index);
